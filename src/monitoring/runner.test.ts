...
describe('runner', () => {
  ...
  it('returns success when status matches expected', async () => {
    ...
    const result = await runCheck(baseCheck, undefined, 5000);
  ...
  it('defaults expectedStatus to 200', async () => {
    ...
    const result = await runCheck(check, undefined, 5000);
    expect(result.success).toBe(true);
  });
  ...
  it('resolves {{WORKER_URL}} placeholder', async () => {
    ...
    await runCheck(check, 'https://moltworker.example.com', 5000);

    expect(mockFetch).toHaveBeenCalledWith(
      'https://moltworker.example.com/sandbox-health',
  ...
  it('passes abort signal and user-agent header', async () => {
    ...
    await runCheck(baseCheck, undefined, 5000);
