...
// We test the gateway status endpoint by building a minimal Hono app that
// mounts the api routes, with DEV_MODE='true' to bypass CF Access auth.

// Dynamic import of the api module so we can test the exported router
import { api } from './api';
...
describe('GET /api/admin/gateway/status', () => {
  ...
  it('returns 500 when an unexpected error occurs', async () => {
    ...
    // so we need a different way to trigger the outer catch — mock listProcesses
    // to throw after the first call
    // Actually, findExistingMoltbotProcess catches errors internally and returns null.
    // So let's just verify it returns stopped when listProcesses fails.
    const app = createTestApp(mockSandbox);
    const res = await makeRequest(app, '/api/admin/gateway/status');
    expect(res.status).toBe(200);
